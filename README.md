...
self-hosted media library with web, android, and desktop clientz that can talk http or p2p. browse & sort music by artist, album, or your own taxonomies. create playlists and radio streams that you can share with your friends. mobile friendly with offline playback support.

_a note on ai:_ this repo has a lot of ai-generated code ...a wild adventure staring into the llm abyss with claude sonnet iv 😎 (both frightening and thrilling at the same time; what a weird time to be a devel). there's no ai features in freqhole. `BEWARE!`

stuff that's here:

1. `/grimoire/` internal-shared lib, rust code; all the p2p transport (via the lovely [iroh crate](https://github.com/n0-computer/iroh/)), sqlx db + biz logic is here

2. `/lib` npm packages + rust crates used both internally and by [freqhole/skein](https://github.com/freqhole/skein) and otherz

   - `/lib/haruspex/` identity/auth lib (webauthn passkeys, invites, api keys, knock federation)
   - `/lib/reliquary/` sqlite + iroh-blobs backed blob storage lib (rust + ts)
   - `/lib/midden/` wasm module for iroh relay transport

3. `/client/spume/` a bunch of js that:
...
4. `/client/charnel/` a desktop & android app built with [tauri](https://v2.tauri.app/) that:
...
5. `/cli/` + `/client/rathole/` a rust cli `cargo run -p cli --bin rathole` that:
...
6. `/server/` a rust server: `cargo run --bin server` that does http stuff:
...
7. misc  
   ...
   c. `/client-codegen/` zod schema generator (derived from rust structs) and a lil' js client that's mainly `fetch()` + zod schema wrapperz for all the api routez. see also: [/client-codegen/README.md](/client-codegen/README.md)

8. `/docs/` probably-stale and out-of-date .md filez here! :feelsgood:

9. `Makefile` for building binz + sqlx migration stuff `make help` to get started.
