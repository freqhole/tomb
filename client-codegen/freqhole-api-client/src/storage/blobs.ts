...
// module-local logger - no-op by default; wire up via setStorageLogger
let logger: (scope: string, ...args: unknown[]) => void = () => {};

/**
 * set the logger function used by blob storage operations.
 * call this at app startup to opt into logging.
 */
export function setStorageLogger(
  fn: (scope: string, ...args: unknown[]) => void,
): void {
  logger = fn;
}
...
export function closeBlobDB(): void {
  if (blobDbInstance) {
    ...
    blobDbInstance = null;
    logger("blobs", "blob database connection closed");
  }
}
...
async function writeBlobToCache(
  blobId: string,
  data: Blob,
  mimeType: string,
): Promise<string> {
...
// revoke object URLs on page unload to avoid memory leaks
...
/**
 * delete blob by ID from both storage and metadata db
 */
