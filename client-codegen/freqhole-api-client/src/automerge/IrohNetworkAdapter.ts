...
// (BiStream, open_bi, accept). the adapter won't work until midden is
// rebuilt with `make build` in the relevant midden directory.
// ---------------------------------------------------------------------------
...
/** console log prefix. */
const TAG = "[freqhole:iroh-adapter]";
...
export interface BiStreamLike {
  ...
  close(): void;
  // raw framing (no length prefix) - used by additional protocols.
  // these methods are optional and only present on some BiStream implementations.
  read_to_end?(max_size: number): Promise<Uint8Array>;
  write_raw_and_finish?(data: Uint8Array): Promise<void>;
}

/**
 * midden stream node interface with bidirectional stream APIs.
 * provides open_bi for outbound connections and accept for inbound.
 */
...
export interface IrohNetworkAdapterOptions {
   ...
   * transport when the first truthy identity arrives.
   * must return an unsubscribe function.
   */
  onIdentityChange?: (cb: (identity: unknown | null) => void) => () => void;

  /**
   * ALPN string to use for automerge sync streams.
   * defaults to SYNC_ALPN ("iroh/automerge-repo/1").
   */
  syncAlpn?: string;
}
...
export class IrohNetworkAdapter extends NetworkAdapter {
   ...
   * and closes any existing stream.
   */
  ...
  /** check whether we have an active stream to a peer (transport-level). */
  ...
  private async checkIdentityAndStart(): Promise<void> {
    ...
    if (identity) {
      ...
      console.log(TAG, "no identity and no onIdentityChange provided, adapter is passive");
    }
  }
