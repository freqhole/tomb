// web locks leader election for a p2p node.
// only one tab per origin should run the node at a time.

/** lock name used for node leader election. */
...
/**
 * attempt to acquire exclusive leadership of the p2p node.
 *
 * behavior:
 ...
 */
export function acquireNodeLeadership(opts: AcquireNodeLeadershipOpts): () => void {
  ...
  navigator.locks.request(LOCK_NAME, { ifAvailable: true }, async (lock: Lock | null) => {
    if (released) return;
    ...
    if (lock !== null) {
      ...
      try {
        await navigator.locks.request(LOCK_NAME, { signal: waitAbort.signal }, async () => {
          if (released) return;
          onStateChange?.("leader");
          await onAcquired();
          await new Promise<void>((resolve) => {
