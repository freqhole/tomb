...
let localLibraryHooks = null;
export function setLocalLibraryHooks(hooks) {
...
async function resolveBlob(item) {
    ...
    if (local) {
        ...
        console.log(`[debug/resolveBlob] ${item.blake3_hash.slice(0, 8)}... resolved from real local library`);
        return local;
    }
    const inMemory = blobCache.get(item.blake3_hash);
    if (inMemory) {
        // TEMP DEBUG - remove once sync-to-local wiring bug is found
        console.log(`[debug/resolveBlob] ${item.blake3_hash.slice(0, 8)}... resolved from in-memory blobCache`);
        return inMemory;
    }
    if (localLibraryHooks) {
        // TEMP DEBUG - remove once sync-to-local wiring bug is found
        console.log(`[debug/resolveBlob] ${item.blake3_hash.slice(0, 8)}... not in real library or memory - host has its own storage, skipping persistent cache, will fetchAndCache/syncToLocal`);
        return null;
    }
    return getCachedMediaBlob(item.blake3_hash);
}
async function fetchAndCache(node, item, onProgress) {
    if (localLibraryHooks?.isSyncEnabled()) {
        try {
            const synced = await localLibraryHooks.syncToLocal(item);
            // TEMP DEBUG - remove once sync-to-local wiring bug is found
            console.log(`[debug/fetchAndCache] ${item.blake3_hash.slice(0, 8)}... syncToLocal returned ${synced ? "a blob (promoted to real library)" : "null"}`);
            ...
            // TEMP DEBUG - remove once sync-to-local wiring bug is found
            console.log(`[debug/fetchAndCache] ${item.blake3_hash.slice(0, 8)}... syncToLocal threw:`, err);
