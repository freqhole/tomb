...
export async function getCachedMediaBlob(blake3Hash) {
    return store.getBlob(blake3Hash, blake3Hash);
}
export async function cacheMediaBlob(blake3Hash, blob) {
    await store.storeBlob(await blob.arrayBuffer(), { filename: blake3Hash, mime: blob.type });
}
export async function evictCachedMediaBlob(blake3Hash) {
