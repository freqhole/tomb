...
const failuresByNodeId = new Map();
export function isRateLimited(nodeId) {
...
export function recordPairingFailure(nodeId) {
    const failures = failuresByNodeId.get(nodeId) ?? [];
    failures.push(Date.now());
    failuresByNodeId.set(nodeId, failures);
}
export function clearPairingFailures(nodeId) {
