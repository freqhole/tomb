...
// design) syncs as a `.m3u8` manifest, additive-only (phase 3). total
// songs/albums/artists synced summary stats are still deferred - not
// wired up here yet.
      ...
      <div class="max-w-2xl mx-auto">
        ...
        <Show when={!loading() && !device()}>
          <div class="p-4 bg-[var(--color-bg-secondary)] border border-[var(--color-border-subtle)] rounded-lg text-sm text-[var(--color-text-muted)]">
            ...
            >
              <span>
                {mountedCount()} device{mountedCount() === 1 ? "" : "s"} mounted, but none selected
                as active yet.
              </span>
            </Show>
            <div class="mt-3">
        ...
        <Show when={!loading() && device() && defaultFilterSet()}>
          {(filterSet) => (
            <div class="mt-4 p-4 bg-[var(--color-bg-secondary)] border border-[var(--color-border-subtle)] rounded-lg">
              ...
              <FilterSetManager filterSetId={filterSet().id} />

              <Show when={syncResult()}>
                {(result) => (
                  <div class="mt-3 pt-3 border-t border-[var(--color-border-subtle)] text-xs text-[var(--color-text-muted)] space-y-2">
                        ...
                        <div>
                          ...
                          <Show when={outcome.failed_songs.length > 0}>
                            <ul class="ml-3 mt-1 list-disc text-red-400/80">
                              <For each={outcome.failed_songs}>{(reason) => <li>{reason}</li>}</For>
                            </ul>
                          </Show>
                        </div>
                      )}
