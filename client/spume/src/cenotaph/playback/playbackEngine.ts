...
let localLibraryHooks: LocalLibraryHooks | null = null;
export function setLocalLibraryHooks(hooks: LocalLibraryHooks | null): void {
  localLibraryHooks = hooks;
}
...
let lastNode: MediaPlaybackNode | null = null;
...
function pruneStaleCacheEntries(): void {
  ...
  for (const hash of blobCache.keys()) {
    if (!live.has(hash)) {
      ...
      void evictCachedMediaBlob(hash).catch(() => {});
    }
  }
  let changed = false;
  const next = new Map(itemStatus());
...
async function ensureCached(node: MediaPlaybackNode, item: MediaRef): Promise<Blob> {
  const resolved = await resolveBlob(item);
  if (resolved) {
    blobCache.set(item.blake3_hash, resolved);
    return resolved;
  }
  setStatusFor(item.blake3_hash, "loading");
  try {
    const blob = await fetchAndCache(node, item);
...
async function prefetchUpcoming(node: MediaPlaybackNode): Promise<void> {
...
async function playItem(node: MediaPlaybackNode, item: MediaRef): Promise<void> {
  lastNode = node;
  ...
  const resolved = await resolveBlob(item);
  setState("buffering");
  setDownloadFraction(resolved || !item.size_bytes ? null : 0);
  try {
    if (!resolved) setStatusFor(item.blake3_hash, "loading");
    const blob =
      resolved ??
      (await fetchAndCache(node, item, (fraction) => {
        if (myGeneration === playGeneration) setDownloadFraction(fraction);
      }));
    if (resolved) blobCache.set(item.blake3_hash, resolved);
    setStatusFor(item.blake3_hash, "ready");
...
export async function play(node: MediaPlaybackNode, item: MediaRef): Promise<void> {
  queue = [item];
  syncQueueSignal();
  await playItem(node, item);
}

export async function replaceQueue(node: MediaPlaybackNode, items: MediaRef[]): Promise<void> {
  queue = [...items];
  syncQueueSignal();
...
export async function appendQueue(node: MediaPlaybackNode, items: MediaRef[]): Promise<void> {
...
export async function skip(node: MediaPlaybackNode): Promise<void> {
...
export async function removeFromQueue(node: MediaPlaybackNode, index: number): Promise<void> {
...
export const mediaPlaybackBackend: PlaybackBackend<MediaPlaybackNode> = {
  play,
  replaceQueue,
  appendQueue,
  pause,
