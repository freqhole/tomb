// AudioDbReviewModal — phase 13b "raw data" review surface for theaudiodb.
//
// dumps the parsed `metadata.audiodb` snapshot for an album as
// pretty-printed json. if no snapshot exists yet, offers an "enqueue"
// button that fires an `AudioDbAlbumDetail` job and polls job status until
...
// album list query is invalidated so the new metadata.audiodb subtree is
...
interface AudioDbReviewModalProps {
  isOpen: boolean;
  onClose: () => void;
  album: AlbumSummary;
  remote: Remote;
...
export function AudioDbReviewModal(props: AudioDbReviewModalProps) {
  // pull audiodb subtree straight from the raw json; survives schema drift.
  const rawAudioDb = createMemo<unknown>(() => {
    const raw = props.album.metadata;
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw);
      return parsed?.audiodb ?? null;
    } catch {
      return null;
    }
  });
  const hasSnapshot = () => rawAudioDb() !== null && rawAudioDb() !== undefined;
  ...
  const onEnqueue = async () => {
    ...
    try {
      const client = await getClientForRemote(props.remote);
      const resp = await client.music.enqueueAudioDbAlbumDetail({
  ...
  return (
    <Modal
      ...
      title={`audiodb — ${props.album.title}`}
      size="xl"
    >
      <div class="p-4 space-y-3">
        <div class="text-xs text-[var(--color-text-muted)]">
          raw <code>metadata.audiodb</code> snapshot for review. nothing here is auto-applied yet —
          this surface is for humans to decide what to promote into structured fields in a later
          phase. lookup is by mbid (release-group) when known, otherwise text-search by artist +
          title.
        </div>
