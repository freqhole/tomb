...
export function FriendsList(props: FriendsListProps) {
  ...
  return (
    <div class="flex flex-col h-full">
      {/* header */}
      <div class="flex items-center justify-between px-3 py-3 flex-shrink-0 bg-[var(--color-bg-primary)]">
      ...
      <div class="flex-1 min-h-0 overflow-y-auto">
        ...
        <Show when={online().length > 0}>
          <div class="py-0.5">
            <For each={online()}>
              {(friend) => (
                <button
                  class="w-full flex items-center gap-2 px-3 py-1.5 min-h-[44px] text-left hover:bg-[var(--color-bg-secondary)]/50 transition-colors"
        ...
        <Show when={offline().length > 0}>
          <button
            class="w-full flex items-center gap-1.5 px-3 py-1.5 min-h-[44px] text-[10px] text-[var(--color-text-tertiary)] hover:text-[var(--color-text-secondary)] transition-colors"
              ...
              {(req) => (
                <div class="px-3 py-1.5">
                  ...
                  <div class="flex gap-2 ml-8 mt-1.5">
                    <button
                      class="text-[11px] text-[var(--color-accent-500)] hover:text-[var(--color-accent-400)] transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
                      onClick={() => props.onAcceptRequest?.(req.node_id)}
                    >
                      accept
                    </button>
                    <button
                      class="text-[11px] text-[var(--color-text-tertiary)] hover:text-red-400 transition-colors min-h-[44px] min-w-[44px] flex items-center justify-center"
