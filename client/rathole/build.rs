...
/// bake the git short sha of the checkout this binary was built from into
/// the binary (read back via `env!("FREQHOLE_GIT_SHA")`) so `/info` can
/// report exactly which commit it came from - mirrors
/// `client/charnel/src-tauri/build.rs`'s identical convention.
fn emit_git_sha() {
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap_or_default();

    // docker builds have no .git in the context - callers pass the sha in
    // as an env var instead (same convention as charnel/spume's builds).
