...
//! freqhole-config.toml (see [`crate::config::ControlSocketConfig`]).
//! shared by both rathole (tty) and charnel (tauri desktop) - each
//! embeds this listener and supplies its own dispatch for the commands
//! it receives over the channel, since the two have very different
//! playback/pairing implementations. see docs/rathole-control-socket.md
//! for the full protocol writeup (originally rathole-only, now shared).
//!
...
pub enum ControlSocketCommand {
    ...
    /// attempt using it for an admin-level grant, then shows the
...
/// one request from a connected control-socket client, sent to
/// whichever app embeds this listener over its own dispatch channel.
/// query commands carry a reply channel; action commands leave it
/// `None` and nothing is written back to the client.
pub struct ControlSocketRequest {
    pub command: ControlSocketCommand,
    /// query commands (`get_state`/`list_audio_devices`) send their
    /// JSON reply here; action commands leave this `None`.
...
/// freqhole config/data dir is active, or which app - rathole or
/// charnel - happens to be running) - `$HOME` is unix-only, matching
/// this module's own `#[cfg(unix)]` gating. the filename keeps its
/// original "rathole-control.sock" name for backward compatibility
/// with existing button-wiring scripts, even now that charnel can
/// listen on it too.
#[cfg(unix)]
...
async fn run(
    ...
    tracing::info!(target: "grimoire::control_socket", path = %path.display(), "control socket listening");
    loop {
                ...
                match lines.next_line().await {
                    Ok(Some(line)) => {
                        ...
                        match parse_command(line) {
                            ...
                            None => {
                                tracing::warn!(target: "grimoire::control_socket", line, "unrecognized control socket command");
                            }
                        }
                    }
                    Ok(None) => break,
                    Err(e) => {
                        tracing::warn!(target: "grimoire::control_socket", error = %e, "control socket read error");
                        break;
                    }
                }
            }
...
mod tests {
    ...
    fn parses_known_commands() {
        ...
        assert_eq!(parse_command("next"), Some(ControlSocketCommand::Next));
        assert_eq!(
            parse_command("previous"),
            Some(ControlSocketCommand::Previous)
        );
        assert_eq!(
            parse_command("volume_up"),
            Some(ControlSocketCommand::VolumeUp)
        );
        assert_eq!(
            parse_command("volume_down"),
            Some(ControlSocketCommand::VolumeDown)
        );
        assert_eq!(parse_command("stop"), Some(ControlSocketCommand::Stop));
        assert_eq!(
            parse_command("show_admin_pin"),
            Some(ControlSocketCommand::ShowAdminPin)
        );
        assert_eq!(
