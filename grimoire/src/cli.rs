...
pub enum MusicBrainzAction {
    ...
    SearchAlbum {
        /// Free-form search query (e.g., "Radiohead Kid A")
        #[arg(long)]
        query: Option<String>,
        /// Artist name (alternative to --query)
        #[arg(long)]
        artist: Option<String>,
        /// Album name (alternative to --query)
        #[arg(long)]
        album: Option<String>,
        /// Maximum results
        #[arg(long, default_value = "10")]
    ...
    /// Search for album WITH cover art (one-step workflow)
    SearchAlbumWithArt {
        /// Free-form search query (e.g., "Radiohead Kid A")
        #[arg(long)]
        query: Option<String>,
    ...
    /// Test MusicBrainz API configuration
    TestConfig,
}
...
pub enum MaintenanceAction {
    ...
    CleanupOrphanedTags {
        /// Perform dry run (don't delete, just report)
        #[arg(long)]
        dry_run: bool,
    },
    ...
    /// Run all orphaned record cleanup tasks
    CleanupAll {
        /// Perform dry run (don't delete, just report)
        #[arg(long)]
        dry_run: bool,
...
async fn handle_music_command(action: MusicAction) -> GrimoireResult<()> {
    ...
    match action {
        ...
        MusicAction::MusicBrainz { action } => {
            handle_musicbrainz_command(action).await?;
        }
    }
...
async fn handle_musicbrainz_command(action: MusicBrainzAction) -> GrimoireResult<()> {
