...
impl MediaBlobCursor {
    ...
    pub fn new(created_at: OffsetDateTime, id: String) -> Self {
        Self { created_at, id }
    }
...
pub struct MediaBlob {
    ...
    /// Indicates whether this blob has binary data stored in database
...
impl MediaBlob {
    ...
    pub fn cursor(&self) -> MediaBlobCursor {
        MediaBlobCursor::new(self.created_at, self.id.clone())
    }
    ...
    pub fn without_data(&self) -> Self {
        ...
        blob.data = None;
        // Preserve has_binary_data flag even when removing data
        blob
    }
...
impl CreateMediaBlob {
    /// Convert to MediaBlob with timestamps (ID will be auto-generated by database)
    pub fn into_media_blob(self) -> MediaBlob {
        let now = OffsetDateTime::now_utc();
        MediaBlob {
            id: String::new(), // Will be auto-generated by database trigger
            data: self.data.clone(),
            has_binary_data: self.data.is_some(),
            sha256: self.sha256,
            size: self.size,
            mime: self.mime,
