-- Fix search_songs function to use correct version column type (BIGINT instead of INTEGER)
-- The songs.version column is BIGINT but the function was declaring it as INTEGER

-- Drop the existing search_songs function
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- Recreate search_songs function with correct version type
...
COMMENT ON FUNCTION search_songs IS 'Enhanced music search function with album-first sorting, fixed array parameter handling, included search_vector in base query, and corrected version column type to BIGINT';
