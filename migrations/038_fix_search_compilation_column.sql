-- Fix search function to remove reference to non-existent is_compilation column
-- This migration fixes the search_songs function that was referencing s.is_compilation which doesn't exist

-- Drop and recreate the search_songs function without the is_compilation reference
...
-- Enhanced search_songs function without is_compilation reference
...
COMMENT ON FUNCTION search_songs IS 'Enhanced song search with comprehensive filter support, fixed to use metadata field for is_compilation instead of non-existent column';
