-- Fix album track ordering to always apply disc/track sorting within albums for all sort types
-- This ensures that songs from the same album are always ordered by disc_number then track_number
-- when they would be adjacent in the result set, regardless of primary sort field

-- Drop the existing search_songs function
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- Recreate search_songs function with proper album track ordering for all sorts
...
BEGIN
    ...
    ORDER BY
        ...
        -- Always apply album-based secondary sorting
        -- When songs from the same album appear together, sort by disc/track
...
COMMENT ON FUNCTION search_songs IS 'Enhanced music search function that always applies proper disc/track ordering within albums for all sort types';
