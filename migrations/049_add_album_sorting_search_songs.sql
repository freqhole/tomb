-- Add album-first sorting to search_songs function
-- This ensures songs are always grouped by album and sorted by disc_number then track_number as secondary sort

-- Drop the existing search_songs function
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- Recreate search_songs function with album-first sorting
...
DECLARE
    ...
    p_tags TEXT[] := ARRAY(SELECT jsonb_array_elements_text(params->'tags'));
    p_tags_any TEXT[] := ARRAY(SELECT jsonb_array_elements_text(params->'tags_any'));
    p_tags_exclude TEXT[] := ARRAY(SELECT jsonb_array_elements_text(params->'tags_exclude'));
    p_genres TEXT[] := ARRAY(SELECT jsonb_array_elements_text(params->'genres'));
    p_artists TEXT[] := ARRAY(SELECT jsonb_array_elements_text(params->'artists'));
    p_albums TEXT[] := ARRAY(SELECT jsonb_array_elements_text(params->'albums'));
...
BEGIN
    ...
    filtered_query AS (
        ...
        WHERE
            ...
            -- array filters
            AND (p_tags IS NULL OR p_tags = '{}' OR bq.tags @> p_tags)
            AND (p_tags_any IS NULL OR p_tags_any = '{}' OR bq.tags && p_tags_any)
            AND (p_tags_exclude IS NULL OR p_tags_exclude = '{}' OR NOT (bq.tags && p_tags_exclude))
            AND (p_genres IS NULL OR p_genres = '{}' OR bq.genre = ANY(p_genres))
            AND (p_artists IS NULL OR p_artists = '{}' OR bq.artist = ANY(p_artists))
            AND (p_albums IS NULL OR p_albums = '{}' OR bq.album = ANY(p_albums))
    )
...
COMMENT ON FUNCTION search_songs IS 'Enhanced music search function with album-first sorting - always sorts by disc_number then track_number as secondary sort after primary field';
