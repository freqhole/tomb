-- migration 064: add video support to feed_eventz
--
-- adds video_id column and extends the feed_type CHECK constraint to include
-- video-related event types: 'favorite_video', 'video_watch'.
--
-- sqlite cannot ALTER a CHECK constraint, so we rebuild the table with the
-- expanded CHECK while preserving all existing data and indexes.
...
INSERT INTO feed_eventz_new SELECT 
    id, feed_type, song_id, album_id, artist_id, playlist_id, session_id,
    NULL as video_id, -- new column, NULL for all existing rows
    created_by_user_id, created_by_username, updated_by_user_id, updated_by_username,
    title, subtitle, description,
    song_ids, images, extra_images, collage_images, genres, tags,
    artist_name, album_title, year, song_count, songs_added, total_duration_ms, image_count, urls,
    rating,
...
-- new indexes for video-related feed events
