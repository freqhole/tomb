-- Fix search_songs function tags extraction issue
-- This migration fixes the "cannot extract elements from a scalar" error

-- Drop the existing search_songs function
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- Recreate search_songs function with proper tags handling
...
DECLARE
    ...
    -- array filters - fixed to handle null and non-array values properly
    p_tags TEXT[] := CASE
        WHEN params ? 'tags' AND jsonb_typeof(params->'tags') = 'array' THEN
...
BEGIN
    ...
    IF p_bpm_max IS NOT NULL THEN
        where_conditions := array_append(where_conditions, 's.bpm <= ' || p_bpm_max);
    END IF;
...
COMMENT ON FUNCTION search_songs IS 'Enhanced music search function with sub_genres support, smart album grouping, and comprehensive filtering (fixed tags extraction issue)';
