-- Fix search_songs function cached plan issues and improve sorting logic
-- Clear cached plans and implement smarter album grouping that only applies when sorting would naturally group songs

-- Drop the existing search_songs function and clear any cached plans
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- Force PostgreSQL to clear any cached plans by disconnecting and reconnecting would be ideal,
-- but we can't do that in a migration. Instead we'll recreate with a slightly different signature
-- to force plan invalidation.

-- Recreate search_songs function with improved sorting logic
...
DECLARE
    ...
    should_group_albums BOOLEAN := p_order_by IN ('album', 'artist', 'album_artist', 'year', 'genre', 'title');
BEGIN
    RETURN QUERY
    WITH base_query AS (
        SELECT
    ...
    ORDER BY
        ...
        -- Secondary album sorting - only for fields that naturally group by album/artist
        CASE WHEN should_group_albums THEN fq.album END ASC NULLS LAST,
        CASE WHEN should_group_albums THEN COALESCE(fq.disc_number, 1) END ASC,
        CASE WHEN should_group_albums THEN COALESCE(fq.track_number, 999) END ASC,

        -- Final fallback sorting for time-based and other fields
        CASE WHEN NOT should_group_albums THEN fq.created_at END DESC
...
COMMENT ON FUNCTION search_songs IS 'Enhanced music search function with smart album grouping that only applies secondary disc/track sorting when the primary sort field would naturally group albums together';

-- Force plan cache invalidation by updating statistics (this is a hint to PostgreSQL)
ANALYZE songs;
