-- Fix individual items missing collection_grid data in social feed
-- This migration removes the restriction that prevented individual items from having collection_grid data
...
BEGIN
    ...
    aggregated_groups AS (
        SELECT
            ...
            -- Collection grid with full song records and metadata (for ALL items - removed individual restriction)
    ...
    SELECT
        ...
        COALESCE(ag.display_domain_ids, ARRAY[]::text[]) as domain_ids,
        ...
        CASE
            ...
            ELSE 'music activity'
        END as subtitle,
        -- Populate image_url from song thumbnail or playlist data
        CASE
            WHEN ag.display_domain_type = 'playlist' THEN
                (SELECT p.thumbnail_blob_id FROM playlists p WHERE p.id::text = ANY(ag.display_domain_ids) LIMIT 1)
            ELSE
                (ag.collection_grid->'songs'->0->>'thumbnail_blob_id')
        END::text as image_url,
        jsonb_build_object(
            'total_songs', (ag.collection_grid->>'total_songs')::int,
            'artist_name', CASE
                WHEN ag.display_domain_type = 'playlist' THEN NULL
                ELSE (ag.collection_grid->'songs'->0->>'artist')
            END,
            'album_name', (ag.collection_grid->'songs'->0->>'album'),
            'playlist_name', CASE
                WHEN ag.display_domain_type = 'playlist' THEN ag.display_title
                ELSE NULL
            END,
            'genre_name', (ag.collection_grid->'songs'->0->>'genre'),
            'description', CASE
                WHEN ag.display_domain_type = 'playlist' THEN
                    (SELECT p.description FROM playlists p WHERE p.id::text = ANY(ag.display_domain_ids) LIMIT 1)
                ELSE NULL
            END,
            'thumbnail_blob_id', CASE
                WHEN ag.display_domain_type = 'playlist' THEN
                    (SELECT p.thumbnail_blob_id FROM playlists p WHERE p.id::text = ANY(ag.display_domain_ids) LIMIT 1)
                ELSE (ag.collection_grid->'songs'->0->>'thumbnail_blob_id')
            END,
            'image_url', CASE
                WHEN ag.display_domain_type = 'playlist' THEN
                    (SELECT p.thumbnail_blob_id FROM playlists p WHERE p.id::text = ANY(ag.display_domain_ids) LIMIT 1)
            ...
            'user_activity', jsonb_build_object(
                ...
                'recent_songs', NULL,
                'period_description', ag.grouping_level,
                ...
                'session_duration',
                    CASE
                        WHEN ag.grouping_level = 'session' THEN
                            EXTRACT(EPOCH FROM (ag.latest_activity - ag.earliest_activity))
                        ELSE 0
                    END,
            ...
            'social_context', jsonb_build_object(
                ...
                'is_trending', ag.min_age_minutes < 60,
                'rating', CASE
                    WHEN ag.primary_event_type = 'rate' THEN (ag.latest_event_data->>'rating')::int
                    ELSE NULL
                END,
                'age_category', CASE
                    WHEN ag.min_age_minutes < 60 THEN 'fresh'
                    WHEN ag.min_age_minutes < 1440 THEN 'recent'
                    WHEN ag.min_age_minutes < 10080 THEN 'weekly'
                    ELSE 'archived'
                END,
                'grouping_level', ag.grouping_level
            ),
        ...
        -- Use actual event time for scoring to maintain chronological order
    ...
    ORDER BY ag.latest_activity DESC  -- Strict chronological order based on actual listening time
