-- Fix created_at sorting by removing forced album grouping for time-based sorts
-- The issue is that grouping by album first completely breaks the primary time-based sort
-- We should sort by the primary field first, then use album/track as tie-breakers only

-- Drop the existing search_songs function
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- Recreate search_songs function with proper time-based sorting
...
BEGIN
    ...
    ORDER BY
        -- Primary sort field first - this is the main sort requested by user
        ...
        -- Secondary sort: album grouping as tie-breaker (only for fields that benefit from it)
        CASE WHEN p_order_by IN ('title', 'artist', 'album', 'album_artist', 'year', 'genre', 'rating', 'user_rating', 'user_is_favorite', 'is_favorite', 'search_rank')
...
COMMENT ON FUNCTION search_songs IS 'Enhanced music search function with proper primary sort first, then album grouping only as tie-breaker for appropriate fields';
