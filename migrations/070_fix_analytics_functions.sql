-- Migration 070: Fix analytics functions to properly join with media_blobs table
-- This migration fixes the get_trending_songs and get_popular_songs_by_period functions
-- to properly join with media_blobs table for file_size and other metadata

-- Drop existing functions first to avoid return type conflicts
DROP FUNCTION IF EXISTS get_trending_songs(integer, integer, text);
DROP FUNCTION IF EXISTS get_popular_songs_by_period(integer, integer, integer);

-- Fix get_trending_songs function with proper media_blobs join
...
BEGIN
    ...
    LEFT JOIN media_blobs mb ON mb.id = ta.media_blob_id
...
-- Fix get_popular_songs_by_period function with proper media_blobs join
...
-- Add documentation
COMMENT ON FUNCTION get_trending_songs IS 'Trending songs analysis with proper media_blobs join for file metadata';
COMMENT ON FUNCTION get_popular_songs_by_period IS 'Popular songs by period with proper media_blobs join for file metadata';
