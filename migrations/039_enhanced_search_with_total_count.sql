-- Enhanced search_songs function with total count and improved sorting support
-- This migration adds total_count to the returned results and expands sorting options
...
-- Enhanced search_songs function with total count support
CREATE OR REPLACE FUNCTION search_songs(
    ...
    version BIGINT,
    search_rank REAL,
    total_count BIGINT  -- NEW: total count of all matching records
) AS $$
BEGIN
    RETURN QUERY
    WITH base_query AS (
        SELECT
            ...
            CASE
                ...
                ELSE 0.0
            END AS search_rank,
            COUNT(*) OVER() AS total_count  -- NEW: get total count using window function
        FROM songs s
    ...
    ORDER BY
        CASE p_order_by
            ...
            WHEN 'duration' THEN EXTRACT(EPOCH FROM bq.duration) * -1  -- NEW: duration support
            WHEN 'duration_seconds' THEN EXTRACT(EPOCH FROM bq.duration) * -1  -- NEW: duration_seconds support
...
COMMENT ON FUNCTION search_songs IS 'Enhanced song search with total count and improved sorting support for year, duration, and duration_seconds fields';

-- Create index on metadata format/codec fields for better file format filtering
CREATE INDEX IF NOT EXISTS idx_songs_metadata_format_codec
ON songs USING GIN ((metadata->'file_format'), (metadata->'codec'));

-- Create index on duration for better duration-based sorting
CREATE INDEX IF NOT EXISTS idx_songs_duration_epoch
ON songs (EXTRACT(EPOCH FROM duration)) WHERE duration IS NOT NULL;
