---
title: prerequisites
description: external tools freqhole needs — ffmpeg and yt-dlp
created: 2026-03-25T09:44:27-04:00
updated: 2026-06-08T14:54:58-04:00
author: "edward sharp"
createdBy: "edwardsharp"
createdSha: "2588f07"
updatedBy: "edwardsharp"
updatedSha: "3f1c9aa"
---

freqhole can use [ffmpeg](https://ffmpeg.org/) and [yt-dlp](https://github.com/yt-dlp/yt-dlp).

these are optional, but recommended.

---

## ffmpeg

`ffmpeg` handles generating audio waveform images and using `ffprobe` for metadata. freqhole will look for it in your PATH and common install locations (`/opt/homebrew/bin`, `/usr/local/bin`, etc.).

**macOS**:

```bash
brew install ffmpeg
```

**Linux**:

```bash
sudo apt install ffmpeg   # debian/ubuntu
sudo dnf install ffmpeg   # fedora
```

**Windows**:

```powershell
winget install --id Gyan.FFmpeg
```

no winget, or prefer a manual install? [this step-by-step guide](https://gist.github.com/edwardsharp/6fc9ce8eba14185225fd31667856f2e3) walks through installing both ffmpeg and yt-dlp binaries by hand.

---

## yt-dlp

needed only if you want freqhole to fetch music from URLs. install via pip or download the binary directly,

**see** [github.com/yt-dlp/yt-dlp](https://github.com/yt-dlp/yt-dlp) for best installation instructions.

```bash
# pip (into a venv or with --break-system-packages on newer distros)
pip install yt-dlp

# or download the binary directly
curl -L https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp -o ~/.local/bin/yt-dlp
chmod +x ~/.local/bin/yt-dlp
```

**Windows**:

```powershell
winget install --id yt-dlp.yt-dlp
```

freqhole will find yt-dlp automatically if it's in your PATH or a common install location. for non-standard paths, set it explicitly in `freqhole-config.toml`:

```toml
[server.fetch_music]
fetch_command = "/full/path/to/yt-dlp ..."
precheck_command = "yt-dlp --print-json --no-download"
```

`precheck_command` enables the add-music review step before the actual download starts. use a metadata-only yt-dlp command here so freqhole can show you what will be fetched first.

---

## mpv

**Linux**: the "experimental" video player will use `mpv` to play video.

```bash
sudo apt install mpv   # debian/ubuntu
sudo dnf install mpv   # fedora
```
