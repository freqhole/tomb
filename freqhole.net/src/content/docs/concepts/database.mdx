...
createdSha: "d0a2360"
updated: 2026-06-08T19:15:48-04:00
updatedBy: "edwardsharp"
updatedSha: "5eeaf1c"
---

there's no central server: each node (charnel desktop app or rathole CLI) owns its music + video data, while the browser client keeps its data in the browser's indexed db. the diagrams on this page currently focus on the older music schema; video uses the same SQLite, blob, and browser storage architecture.

data is split across four stores:

- `data/grimoire.db` - the main music + video metadata **SQLite** database.
...
- `data/freqhole-blobz/` - an **iroh-blobs** content-addressed store for music, video, and image bytes, with its own `blobs.db` index keyed by blake3 hash.
...
`grimoire.db` holds metadata only - no raw media bytes. `media_blobz` rows point at content stored elsewhere: thumbnails and waveforms in `grimoire-blobdata.db`, originals in the iroh-blobs store.
...
erDiagram
  ...
  songz {
    ...
    TEXT created_by
    TEXT updated_by
    TEXT media_blob_sha256
    TEXT media_blob_blake3
    TEXT media_blob_mime
    INTEGER media_blob_size
  }
  artist_albumz {
    TEXT artist_id FK
    TEXT album_id FK
...
erDiagram
  ...
  videoz ||--o{ feed_eventz : "video_id (-> videoz)"
  playback_sessionz ||--o{ feed_eventz : "session_id (-> playback_sessionz)"
...
erDiagram
  ...
  external_storage_claimed_pathz {
    TEXT device_id PK
    TEXT relative_path PK
    INTEGER claimed_at
  }
  external_storage_device_statz {
    TEXT device_id PK
    INTEGER last_synced_at
  }
  external_storage_filter_set_filterz {
    TEXT id PK
    TEXT filter_set_id FK
    TEXT filter_type
    TEXT mode
    TEXT artist_id FK
    TEXT album_id FK
    ...
    INTEGER criteria_value
    INTEGER created_at
    INTEGER criteria_scope
  }
  external_storage_filter_setz {
    TEXT id PK
    TEXT name
    INTEGER created_at
    INTEGER updated_at
  ...
  external_storage_sync_manifestz {
    TEXT device_id PK
    TEXT sync_set_id PK
    TEXT filename
    INTEGER synced_at
  }
  external_storage_synced_songz {
    TEXT device_id PK
    TEXT song_id PK
    TEXT relative_path
    TEXT sha256
  ...
  import_session_send_targetz {
    TEXT session_id PK
    TEXT target_remote_id
    TEXT target_remote_name
    INTEGER created_at
  }
  play_eventz {
    TEXT id PK
  ...
  playback_progressz {
    ...
    TEXT entity_type PK
    TEXT entity_id PK
    REAL position_fraction
    REAL position_seconds
    REAL duration_seconds
    TEXT position_locator
    INTEGER completed_at
    INTEGER updated_at
  }
  playback_sessionz {
    ...
    INTEGER total_duration_ms
    INTEGER played_duration_ms
    INTEGER current_item_index
    INTEGER current_item_position_ms
    TEXT status
    INTEGER created_at
    INTEGER updated_at
  }
  ...
  webauthn_challenges {
    ...
    TEXT user_id
    TEXT username
    INTEGER is_account_link
    TEXT invite_code
    INTEGER created_at
    INTEGER expires_at
  }
  media_blobz ||--o{ entity_imagez : "media_blob_id (-> media_blobz)"
  tagz ||--o{ entity_tagz : "tag_id (-> tagz)"
  taxonz ||--o{ entity_taxonz : "taxon_id (-> taxonz)"
  playlistz ||--o{ external_storage_filter_set_filterz : "playlist_id (-> playlistz)"
  songz ||--o{ external_storage_filter_set_filterz : "song_id (-> songz)"
  tagz ||--o{ external_storage_filter_set_filterz : "tag_id (-> tagz)"
  taxonz ||--o{ external_storage_filter_set_filterz : "taxon_id (-> taxonz)"
  albumz ||--o{ external_storage_filter_set_filterz : "album_id (-> albumz)"
  artistz ||--o{ external_storage_filter_set_filterz : "artist_id (-> artistz)"
  external_storage_filter_setz ||--o{ external_storage_filter_set_filterz : "filter_set_id"
  job_sessionz ||--o{ import_blobz : "session_id (-> job_sessionz)"
  job_sessionz ||--o{ import_session_send_targetz : "session_id (-> job_sessionz)"
  user_accountz ||--o{ play_eventz : "user_id (-> user_accountz)"
  radio_stationz ||--o{ play_eventz : "radio_station_id (-> radio_stationz)"
  playlistz ||--o{ play_eventz : "playlist_id (-> playlistz)"
  media_eventz ||--o{ play_eventz : "media_event_id (-> media_eventz)"
  user_accountz ||--o{ playback_progressz : "user_id (-> user_accountz)"
  user_accountz ||--o{ playback_sessionz : "user_id (-> user_accountz)"
  playlistz ||--o{ playlist_itemz : "playlist_id (-> playlistz)"
  media_blobz ||--o{ video_seasonz : "poster_blob_id (-> media_blobz)"
  video_seriez ||--o{ video_seasonz : "series_id"
  media_blobz ||--o{ video_seriez : "poster_blob_id (-> media_blobz)"
  media_blobz ||--o{ videoz : "poster_blob_id (-> media_blobz)"
  media_blobz ||--o{ videoz : "media_blob_id (-> media_blobz)"
  video_seasonz ||--o{ videoz : "season_id"
  video_seriez ||--o{ videoz : "series_id"
```

_full-text search mirrors (FTS5): `albumz`, `artistz`, `playlistz`, `songz`, `taxonz`, `video_seriez`, `videoz` each have a companion `<table>_fts` virtual table kept in sync by triggers._
...
- **`freqhole_blobs`** — metadata for locally-stored images, music, and video. the actual bytes live in OPFS or the Cache API, not in IndexedDB.
...
erDiagram
  ...
  user_peer_nodes {
    key node_id PK "node_id"
    index by_user_id "user_id"
    index by_created_at "created_at"
  }
```

#### `freqhole_music` · catalog (v19)
...
erDiagram
  ...
  songs {
    ...
    index by_file_identity "[file_name, file_size, last_modified]"
    index by_album_disc_track "[album_id, disc_number, track_number]"
...
#### `freqhole_music` · tags + taxonomy (v19)
...
erDiagram
  ...
  taxons {
    ...
    index by_remote_kind_slug UK "[remote_id, kind_slug, slug]"
...
#### `freqhole_music` · user library (v19)
...
#### `freqhole_music` · other (v19)

```mermaid
erDiagram
  playlist_items {
    key playlist_id_entity_type_entity_id PK "[playlist_id, entity_type, entity_id]"
    index by_playlist_id "playlist_id"
  }
  entity_taxons {
    key entity_type_entity_id_taxon_id PK "[entity_type, entity_id, taxon_id]"
    index by_entity_type "entity_type"
    index by_entity_id "[entity_type, entity_id]"
    index by_taxon_id "[entity_type, taxon_id]"
    index by_remote_id "remote_id"
    index by_created_at "created_at"
  }
  taxon_kinds {
    key domain_kind_slug PK "[domain, kind_slug]"
    index by_domain "domain"
  }
  import_review_sessionz {
    key session_id PK "session_id"
    index by_created_at "created_at"
  }
  import_review_blobz {
    key song_id PK "song_id"
