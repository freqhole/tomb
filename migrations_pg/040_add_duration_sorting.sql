-- Add duration sorting support to search_songs function
-- This migration updates the search_songs function to support sorting by duration_seconds

-- drop the existing search_songs function so we can recreate it with duration sorting
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- recreate search_songs function with duration sorting support
...
COMMENT ON FUNCTION search_songs IS 'search songs with jsonb parameter and duration sorting support - user-aware and much easier to use than positional parameters';
