-- Fix user preference sorting in search_songs function
-- This adds missing ORDER BY cases for user_is_favorite and ensures user_rating works correctly

-- Drop the existing search_songs function
DROP FUNCTION IF EXISTS search_songs CASCADE;

-- Recreate search_songs function with proper user preference sorting
...
BEGIN
    ...
    ORDER BY
        ...
        CASE WHEN p_order_by = 'user_rating' AND is_asc THEN fq.rating END ASC,
        CASE WHEN p_order_by = 'user_rating' AND NOT is_asc THEN fq.rating END DESC,
        CASE WHEN p_order_by = 'user_is_favorite' AND is_asc THEN fq.is_favorite END ASC,
        CASE WHEN p_order_by = 'user_is_favorite' AND NOT is_asc THEN fq.is_favorite END DESC,
        CASE WHEN p_order_by = 'is_favorite' AND is_asc THEN fq.is_favorite END ASC,
        CASE WHEN p_order_by = 'is_favorite' AND NOT is_asc THEN fq.is_favorite END DESC,
        CASE WHEN p_order_by = 'duration_seconds' AND is_asc THEN EXTRACT(EPOCH FROM fq.duration) END ASC,
        CASE WHEN p_order_by = 'duration_seconds' AND NOT is_asc THEN EXTRACT(EPOCH FROM fq.duration) END DESC,
...
COMMENT ON FUNCTION search_songs IS 'search songs with user preference sorting support - handles user_rating and user_is_favorite sort fields properly';
