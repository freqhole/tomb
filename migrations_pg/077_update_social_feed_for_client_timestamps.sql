-- Update social feed function to use client timestamps for proper chronological ordering
-- This migration updates the get_social_feed_items function to use client_timestamp when available
...
BEGIN
    ...
    progressive_groups AS (
        SELECT
            ...
            sb.session_group,
            sb.media_blob_id,
            -- Progressive temporal grouping based on actual listening time
            CASE
                WHEN sb.age_minutes < 10 THEN
                    'individual_' || sb.user_id::text || '_' || sb.event_timestamp::text
                ...
                WHEN sb.age_minutes < 1440 THEN
                    'daily_' || sb.user_id::text || '_' || DATE(sb.event_timestamp)::text
                -- Weekly grouping: 1 day - 1 week (group by week)
                WHEN sb.age_minutes < 10080 THEN
                    'weekly_' || sb.user_id::text || '_' || DATE_TRUNC('week', sb.event_timestamp)::text
                -- Monthly grouping: 1 week - 1 month (group by month)
                WHEN sb.age_minutes < 43200 THEN
                    'monthly_' || sb.user_id::text || '_' || DATE_TRUNC('month', sb.event_timestamp)::text
                -- Yearly grouping: 1+ months (group by year)
                ELSE
                    'yearly_' || sb.user_id::text || '_' || DATE_TRUNC('year', sb.event_timestamp)::text
            END as grouping_key,
    ...
    aggregated_groups AS (
        SELECT
            ...
            -- Collection grid with full song records and metadata (for grouped items only)
            CASE
...
COMMENT ON FUNCTION get_social_feed_items IS 'returns social feed in chronological order using client timestamps when available, with progressive temporal grouping (10min threshold)';
